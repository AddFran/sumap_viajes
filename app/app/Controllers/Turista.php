...
public function guardarReserva()
{
   ...
   public function confirmarPago()
    {
        ...
        // Verificar si hay suficiente cupo para la reserva
        $experiencia = $experienciaModel->find($id_experiencia);

        if (!$experiencia || $experiencia['cupo_maximo'] < $numero_personas) {
            // Si no hay suficiente cupo, mostrar error
            return redirect()->back()->with('error', 'No hay suficiente cupo disponible.');
        }

        // Simulación de un proceso de pago (puedes integrarlo con un gateway de pago real)
        // Crear pago
        $id_pago = $pagoModel->insert([
            'monto' => $monto,
            'metodo_pago' => $metodo,
            'estado_pago' => 'completado' // Cambiar dependiendo del resultado real del pago
        ], true);

        // Si el pago fue exitoso, creamos la reserva
        if ($id_pago) {
            // Crear reserva
            $reservaModel->insert([
                'id_usuario' => $session->get('id_usuario'),
            ...
            return redirect()->back()->with('error', 'El pago no se pudo procesar, intente nuevamente.');
        }
    }
   ...
   public function cancelarReserva()
    ...
    if ($experiencia && isset($experiencia['cupo_maximo'])) {
